...
const singleUserDetails = async (req, res) => {
  try {
    ...
    console.log(
      incidentDetails,
      "--------------This is the incident deatils after fetching using id."
    );
...
const assignIncident = async (req, res) => {
  try {
    ...
    const Incident = await IncidentSchema.findByIdAndUpdate(incident, {
      userassigned: user,
    });
    ...
    const User = await UserSchema.findByIdAndUpdate(user, {
...
const resolveIncident = async (req, res) => {
  try {
    ...
    const resolvedIncident = await IncidentSchema.findByIdAndUpdate(
      req.params.id,
      { resolved: true }
    );
    res.status(200).json({
